...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False):
        if self.gamma_1 is None:
            if return_attention:
                attn_out, attn_map = self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias, return_attention=True)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, return_attn=False):
        ...
        for blk in self.blocks:
            if return_attn:
                x, attn_map = blk(x, rel_pos_bias=rel_pos_bias, return_attention=True)
        ...
        if self.fc_norm is not None:
            t = x[:, 1:, :]
            features = self.fc_norm(t.mean(1))
        else:
            features = x[:, 0]
        if return_attn:
    ...
    def forward(self, x, return_attn=False):
        if return_attn:
            features, attn_map = self.forward_features(x, return_attn=True)
            x = self.head(features)
            return features, x, attn_map
        else:
